...
const SplineComponent = () => {
  ...
  if (hasError) {
    return (
      <div className="absolute inset-0 bg-gradient-to-br from-background/50 via-background/10 to-background/50 animate-gradient" />
    )
  }
...
export default function DashboardLayout({
  ...
  return (
    <div className="relative min-h-screen w-full p-2 sm:p-4">
      {/* Background with fallback */}
      <div className="fixed bottom-0 right-0 z-0 h-[80vh] w-[80vw]">
        <Suspense
          fallback={
            <div className="absolute inset-0 bg-gradient-to-br from-background/50 via-background/10 to-background/50 animate-gradient" />
          }
      ...
      {/* Main Layout */}
      <div className="relative z-10 flex min-h-[calc(100vh-theme(spacing.8))] rounded-lg border border-border bg-black/20 backdrop-blur-sm drop-shadow-md overflow-hidden">
        {/* Sliding Sidebar */}
        ...
        <div className="flex-1 flex flex-col min-w-0 pl-16 lg:pl-0 relative">
          {/* Top Header */}
          <header className="h-16 border-b border-border flex items-center justify-between px-4 bg-background/50 backdrop-blur-sm sticky top-0 z-30">
            <div className="flex-1" />
            <div className="hidden sm:flex items-center gap-2 bg-secondary rounded-lg px-3 py-1.5 text-sm text-muted-foreground hover:text-foreground transition-colors">
            ...
            <div className="flex-1 flex justify-end items-center gap-2 sm:gap-4">
              ...
              <DropdownMenu>
                <DropdownMenuTrigger className="flex items-center gap-2 bg-secondary rounded-lg px-2 sm:px-3 py-2 text-sm hover:bg-secondary/80 transition-colors">
